...
// Page table flags
#define MEMORY_PAGE_TABLE_PRESENT      0x0001   // 1=table present
#define MEMORY_PAGE_TABLE_WRITE        0x0002   // 1=write access
#define MEMORY_PAGE_TABLE_USER_MODE    0x0004   // 0=supervisor mode, 1=user mode
#define MEMORY_PAGE_TABLE_WRITETHROUGH 0x0008   // write-through
#define MEMORY_PAGE_TABLE_CACHED       0x0010   // cached
#define MEMORY_PAGE_TABLE_ACCESSED     0x0020   // accessed
#define MEMORY_PAGE_TABLE_RESERVED     0x0040   // reserved bit - must be 0
#define MEMORY_PAGE_TABLE_SIZE         0x0080   // 0=4Kb page
#define MEMORY_PAGE_TABLE_GLOBAL       0x0100   // global table
...
void k_initMemory(PageDirectory* pPageDir)
{
   ...
   // map the directory
   //k_realMapAddr(pPageDir, (unsigned int)pPageDir, (unsigned int)pPageDir);
   //k_printf("mapped kernel page directory 0x%x\n", pPageDir);

   // map the first table
   //k_realMapAddr(pPageDir, (unsigned int)pTable, (unsigned int)pTable);
   //unsigned int tableIndex = GET_PAGE_TABLE_INDEX((unsigned int)pTable);
   //k_printf("page table (log) 0x%x\n", pTable);
   //if (GET_DISK_LOCATION((unsigned int)pTable) != GET_DISK_LOCATION(pPageDir[0]))
  // {
   //   k_printf("mapping failed! (0x%x != 0x%x)\n", GET_DISK_LOCATION((unsigned int)pTable), GET_DISK_LOCATION(pPageDir[0]));
   //   while (1);
   //}
   ...
   {
      k_realMapAddr(pPageDir, i, i);
   }
  
   // set the page directory to CR3
   k_setPageDirectory(pPageDir);

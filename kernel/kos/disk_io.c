...
#define grub_printf k_printf
#define grub_strlen k_strlen
#define grub_strcpy k_strcpy
#define grub_strcmp k_strcmp
...
unsigned int current_drive = GRUB_INVALID_DRIVE;
...
int get_diskinfo(int drive, struct geometry* pGeometry)
{
   return 0;
}
...
rawread (int drive, int sector, int byte_offset, int byte_len, char *buf)
{
  ...
  while (byte_len > 0 && !errnum)
	...
	sectors_per_vtrack = buf_geom.sectors;
      ...
      if (track != buf_track)
	{
	  int disk_err, read_start = track, read_len = sectors_per_vtrack;
	  ...
	  disk_err = biosdisk (BIOSDISK_READ, drive, &buf_geom, read_start, read_len, BUFFERSEG);
	  if (disk_err)
	    {
	      buf_track = -1;

	      if (disk_err == BIOSDISK_ERROR_GEOMETRY)
		errnum = ERR_GEOM;
	      else
		{
		  /*
